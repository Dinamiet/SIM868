...
void SIM868_Bearer_SetConnectionType(ATTerminal* at, uint8_t identifier, SIM868BearerConnection type)
{
	...
	sprintf(command, "AT+SAPBR=3,%d,\"Contype\",\"%s\"", identifier, type == SIM868_BEARER_CONNECTION_GPRS ? "GPRS" : "CSD");
...
{
	char command[MAX_COMMAND_SIZE];
	sprintf(command, "AT+SAPBR=3,%d,\"APN\",\"%s\"", identifier, apn);
	ATTerminal_SendCommand(at, command);
}
...
{
	char command[MAX_COMMAND_SIZE];
	sprintf(command, "AT+SAPBR=1,%d", identifier);
	ATTerminal_SendCommand(at, command);
}
